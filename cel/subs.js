...
async function testSub(c){
	let oid=randidinit;
	let id=oid;
	let tid=7;
	let rid=tid-id.length;
//	let token=await askQuestion("enter token\n");
        let token=fs.readFileSync("puretoken.txt","utf8").toString().replaceAll(/\s/gi,"");
	console.log("TOKEN :\n");
	console.log(token);
...
if(process.argv.length>3)randidinit=process.argv[3];
